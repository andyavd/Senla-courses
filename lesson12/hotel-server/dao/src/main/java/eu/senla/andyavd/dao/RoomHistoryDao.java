...
import eu.senla.andyavd.utils.common.DateFormatter;
...
public class RoomHistoryDao extends GenericDao<RoomHistory> implements IRoomHistoryDao {
	...
	public List<Service> getVisitorServices(Connection connection, int visitorId) throws Exception {
		...
		try (PreparedStatement statement = connection.prepareStatement(
				"SELECT * FROM service WHERE service.id IN (SELECT service_id FROM visitor_service WHERE history_id = (SELECT history.id FROM history WHERE visitor_id = "
	...
	public List<Visitor> getLastVisitorsOfRoom(Connection connection, int roomId) throws Exception {
		...
		try (PreparedStatement statement = connection.prepareStatement(
				"SELECT visitor.id, visitor_name FROM visitor, history WHERE visitor.id = history.visitor_id AND room_id = "
